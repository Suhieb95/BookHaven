...
public class UserRegistrationService(IUnitOfWork unitOfWork, IWebHostEnvironment env, INotificationService notificationService, IOptions<EmailSettings> emailSettings, IJwtTokenGenerator jwtTokenGenerator) : IUserRegistrationService
{
    ...
    public async Task<Result<bool>> ConfirmEmailAddress(Guid id, CancellationToken? cancellationToken = null)
    {
        ...
        await EmailHelpers.SendEmailConfirmedNotify(user!.EmailAddress, _emailSettings.SuccessURL, _env, cancellationToken, _notificationService);
    ...
    private async Task SendEmailConfirmationLink(CancellationToken? cancellationToken, Guid id)
    {
        ...
        await EmailHelpers.SendConfirmationEmail(user!.EmailAddress, id, _emailSettings.EmailConfirmationURL, _env, cancellationToken, _notificationService);
    }
    private async Task GenerateAndSendSetPassowrdLink(string to, Guid id, CancellationToken? cancellationToken)
    {
        ...
        await EmailHelpers.SendResetPasswordLink(to, id, _emailSettings.ResetPasswordURL, _env, cancellationToken, _notificationService);
    }
}